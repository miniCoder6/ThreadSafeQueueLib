...
static void BM_MPSC(benchmark::State& state) {
    ...
    for (auto _ : state) {
        ...
        for (int p = 0; p < producers; ++p) {
            producer_threads.emplace_back([&] {
                ...
                for (size_t i = 0; i < MPSC_OPS_PER_PRODUCER; ++i) {
                    q.push(i);
                }
            });
        }
        ...
        std::thread consumer([&] {
            ...
            int value;
            const size_t total = producers * MPSC_OPS_PER_PRODUCER;
    ...
    state.SetItemsProcessed(state.iterations() * producers * MPSC_OPS_PER_PRODUCER);
}
