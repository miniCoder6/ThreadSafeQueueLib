...
template <typename T> class lockfree_mpsc_unbounded {
  // Works exactly same as the blocking_mpsc_unbounded queue (see this once)
  ...
  // [Copy of blocking_mpsc_unbounded]
  ...
  // Queue moves values between threads
  // so T must support moving.
  static_assert(std::is_move_constructible_v<T>, "T must be move constructible");

  // We assign into node storage during push/emplace.
  static_assert(std::is_move_assignable_v<T>, "T must be move assignable");

  // if move throws during push, queue state may become inconsistent.
  static_assert(std::is_nothrow_move_constructible_v<T>, "T must be nothrow move constructible");

  // current stub-node design default-constructs nodes: node* n = new node();
  // so T must also be default constructible.
